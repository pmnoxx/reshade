...
# This script builds the 64-bit version of ReShade and optionally all example addons
...
Options:
    ...
    -BuildExamples            Build all example addons in addition to ReShade
...
Examples:
    ...
    .\build.ps1 -BuildExamples                    # Build ReShade + all examples
    .\build.ps1 -BuildExamples -Configuration Debug # Build Debug + examples
...
if ($BuildExamples) {
    Write-Host "Examples: Building all example addons" -ForegroundColor Yellow
} else {
    Write-Host "Examples: Not building (use -BuildExamples to include)" -ForegroundColor Gray
}
...
function Build-Examples {
    ...
    Write-Host "Building ReShade Examples..." -ForegroundColor Yellow

    # Check if Examples.sln exists
    if (-not (Test-Path "examples\Examples.sln")) {
        Write-Warning "Examples.sln not found in examples directory. Skipping examples build."
        return $true
    }

    # Build arguments for examples
    $buildArgs = @(
    ...
    # Build command
    Write-Host "Executing MSBuild for examples with arguments: $($buildArgs -join ' ')" -ForegroundColor Gray
    ...
    if ($process.ExitCode -eq 0) {
        Write-Host "Examples build completed successfully in $($duration.ToString('mm\:ss'))!" -ForegroundColor Green
        return $true
    } else {
        Write-Error "Examples build failed with exit code: $($process.ExitCode)"
        return $false
    }
}
...
function Show-BuildResults {
    ...
    if (Test-Path $outputDir) {
        ...
        # Show example files if BuildExamples was used
        if ($BuildExamples) {
            Write-Host "`nExample addon files:" -ForegroundColor Yellow
            $exampleFiles = Get-ChildItem -Path "examples" -Recurse -Filter "*.dll" | Where-Object {
                $_.DirectoryName -like "*$Configuration*" -or $_.DirectoryName -like "*x64*"
            }
            ...
            if ($exampleFiles) {
                ...
                Write-Host "  No example DLLs found in expected output directories" -ForegroundColor Yellow
            }
        }
    } else {
...
try {
    ...
    if ($success) {
        ...
        if ($BuildExamples) {
            ...
            $examplesSuccess = Build-Examples -MSBuildPath $msbuildPath -Configuration $baseConfig
            if (-not $examplesSuccess) {
                Write-Warning "Examples build failed, but ReShade build succeeded."
                Write-Host "You can try building examples separately or check the error messages above." -ForegroundColor Yellow
            }
        }
        ...
        if ($BuildExamples) {
            Write-Host "ReShade 64-bit and examples have been built and are ready for use." -ForegroundColor Cyan
            Write-Host "`nNext steps:" -ForegroundColor Yellow
            Write-Host "  - Copy ReShade64.dll to your game directory" -ForegroundColor Gray
            Write-Host "  - Copy example addon DLLs to your game directory" -ForegroundColor Gray
            ...
            Write-Host "  - Use inject.exe to inject into running processes" -ForegroundColor Gray
            Write-Host "  - fxc.exe can compile .fx shader files" -ForegroundColor Gray
            Write-Host "  - Use -BuildExamples to also build example addons" -ForegroundColor Gray
        }
    } else {
